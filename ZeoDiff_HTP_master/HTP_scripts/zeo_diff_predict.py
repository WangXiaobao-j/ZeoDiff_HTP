...
class ZeoliteGUI:
    ...
    def create_feature_script(self, input_path, output_path, is_batch=False):
        ...
        if is_batch:
            processor_class = "BatchZeoliteProcessor"
            # Use repr() to ensure proper escaping of Windows paths
            params = f'input_folder={repr(input_path)}, output_file={repr(output_path)}'
        else:
            processor_class = "ZeoliteFeatureExtractor"
            # Use repr() to ensure proper escaping of Windows paths
            params = f'cif_file={repr(input_path)}, output_dir={repr(output_path)}, auto_remove_symmetry=True'
